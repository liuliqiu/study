...
def p84(n):
    ...
    for i in range(40):
        M[10][i]+=M[30][i]
        M[30][i]=0
        M[0][i]+=(M[2][i]+M[17][i]+M[33][i]+M[7][i]+M[22][i])/16+M[36][i]/15
        M[10][i]+=(M[2][i]+M[17][i]+M[33][i]+M[7][i]+M[22][i])/16+M[36][i]/15
        M[2][i]*=(7/8)
        M[17][i]*=(7/8)
        M[33][i]*=(7/8)
    ...
    result=sorted([(i,j) for i,j in enumerate(result)],key=lambda x:x[1],reverse=True)
    print(result)
p84(6)
